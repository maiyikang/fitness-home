...
def build_structural_bin(
    ...
    raw_checks = md["constraint_checks"]
    ...
    if constraints.get("min_fiber") is not None or "fiber" in checks:
        check_names.append("fiber")

    missing = [name for name in check_names if name not in checks]
    if missing:
        raise KeyError(
            f"Missing constraint checks {missing} in sample "
            f"{train_row.get('sample_id')} with keys={list(raw_checks.keys())}"
        )

    failed = [name for name in check_names if not checks[name]]
    ...
    if constraints.get("min_fiber") is not None:
        required_numeric["fiber"] = constraints.get("min_fiber")

    numeric_margins = {}
    for name, threshold in required_numeric.items():
        if threshold is None:
            continue
        if name not in avgs:
            raise RuntimeError(
                f"Could not parse Average {name} from sample "
                f"{train_row.get('sample_id')}"
            )
    ...
    if not numeric_margins:
        raise RuntimeError(
            f"No numeric constraints found in sample "
            f"{train_row.get('sample_id')}"
        )
    ...
    features = {
        ...
        "structural_bin": key,
        "active_constraint_checks": check_names,
    }
    return key, features
