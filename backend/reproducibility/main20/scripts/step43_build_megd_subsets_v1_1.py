...
def build_structural_bin(train_row: dict[str, Any], boundary_threshold: float) -> tuple[str, dict[str, Any]]:
    ...
    def check_value(name: str) -> bool:
        ...
        if name == "fiber" and "fibre" in checks:
            return bool(checks["fibre"])
        raise KeyError(f"Missing constraint check '{name}' in sample {train_row.get('sample_id')} with keys={list(checks.keys())}")

    failed = [k for k in ("cuisine", "calories", "protein", "fiber") if not check_value(k)]
    ...
    cuisine_mismatch = not check_value("cuisine")
